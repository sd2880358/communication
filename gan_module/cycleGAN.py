...
def start_train(BATCH_SIZE, BUFFER_SIZE, data, filePath):
    ...
    def train_step(total, label):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            fake_u = generator_u(g_noise, training=True)
            gen = (s + fake_u)
            ...
            total_s_loss = gen_s_loss  + gen_loss * 0.5
            total_u_loss =  gen_u_loss + gen_loss * 0.5
        ...
        disentangle_t_optimizer.apply_gradients(zip(gradients_of_disentangle_t,
                                                    disentangle_t.trainable_variables))
